...
"""
    ...
    code = re.sub(r'\s*onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value \}\)\}\s*(required)?\s*(type="text"|type="email")?\s*value=\{form\.[a-zA-Z]+\}', r1, code)
    code = re.sub(r'\s*onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value as ApiBranchStatus \}\)\}\s*(required)?\s*value=\{form\.[a-zA-Z]+\}', r1, code)

    # Fix aria-invalid if any manually handled fieldErrors, wait there aren't any.

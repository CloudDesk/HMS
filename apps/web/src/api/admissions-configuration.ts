...
export type Ward = { id: string; branch_id: string; name: string; ward_type: string; room_type: string | null; floor: string; capacity: number | null; description: string | null; status: WardStatus; created_at: string; updated_at: string };
export type Bed = { id: string; branch_id: string; ward_id: string; ward_name: string; bed_number: string; bed_category: string; bed_type: string; charge_category: string; gender_restriction: BedGenderRestriction; room_number: string | null; status: BedStatus; created_at: string; updated_at: string };
...
export const admissionsConfigurationApi = {
  ...
  updateBed: (id: string, body: UpdateBedPayload) => apiClient.request<Bed>(`/admissions/beds/${id}`, { method: 'PATCH', body }),

...
export function BillingWorkspacePage() {
  ...
  const {
    ...
    queries: { branches, patientsQuery, visitsQuery, servicesQuery, batchesQuery, invoiceQuery, paymentsQuery },
    ...
    return <div className="billing-page">
      ...
      <div className="billing-workspace-grid">
        <main>
          <section className="billing-card">
            ...
            <div className="billing-form-grid">
              ...
              <label><span>Patient *</span><select {...invoiceForm.register('patient_id', { onChange: () => invoiceForm.setValue('visit_id', '') })}><option value="">Select patient</option>{patientsQuery.data?.data.map((patient) => <option key={patient.id} value={patient.id}>{patient.patient_number} Â· {patient.first_name} {patient.last_name}</option>)}</select><small>{invoiceForm.formState.errors.patient_id?.message}</small></label>
              <label><span>OPD Visit *</span><select disabled={!selectedPatient || !selectedBranch || visitsQuery.isLoading} {...invoiceForm.register('visit_id')}><option value="">{visitsQuery.isLoading ? 'Loading visitsâ€¦' : 'Select visit'}</option>{visitsQuery.data?.data.map((visit) => <option key={visit.id} value={visit.id}>{visit.visit_number} Â· {formatBillingDate(visit.visit_date)} Â· {visit.doctor_name}</option>)}</select><small>{invoiceForm.formState.errors.visit_id?.message}</small></label>
            ...
            <form className="billing-item-builder" onSubmit={addItem}>
              ...
              <label><span>Service / Medicine</span><select disabled={selectedSource === 'PHARMACY' ? batchesQuery.isLoading : servicesQuery.isLoading} {...itemForm.register('service_id')}><option value="">{(selectedSource === 'PHARMACY' ? batchesQuery.isLoading : servicesQuery.isLoading) ? 'Loading...' : 'Select item'}</option>
                {selectedSource === 'PHARMACY'
                  ? batchesQuery.data?.data.map((batch) => <option key={batch.id} value={batch.id}>{batch.medicine?.name} (Batch: {batch.batch_number}) Â· {formatBillingMoney(batch.unit_price)}</option>)
                  : servicesQuery.data?.data.map((service) => <option key={service.id} value={service.id}>{service.name} Â· {formatBillingMoney(service.standard_price)}</option>)}
        ...
        <aside className="billing-summary-card">
          ...
          <div className="billing-total-row"><span>Subtotal</span><strong>{formatBillingMoney(draftSubtotal)}</strong></div><div className="billing-total-row"><span>Discount</span><strong>âˆ’ {formatBillingMoney(invoiceForm.watch('discount_amount'))}</strong></div><div className="billing-total-row"><span>Tax</span><strong>{formatBillingMoney(invoiceForm.watch('tax_amount'))}</strong></div><div className="billing-total-row grand"><span>Total</span><strong>{formatBillingMoney(Math.max(0, draftTotal))}</strong></div>
          <button className="btn-primary billing-full-button" disabled={!canCreate || createMutation.isPending} onClick={createInvoice} type="button">{createMutation.isPending ? 'Creatingâ€¦' : 'Create Invoice Draft'}</button>
        </aside>
      </div>
    </div>;
  }
  if (invoiceQuery.isLoading) return <div className="billing-state"><span className="loading-spinner" /> Loading invoiceâ€¦</div>;
  ...
  return <div className="billing-page">
    ...
    <div className="billing-workspace-grid">
      ...
      <aside className="billing-summary-card">
        ...
        <div className="billing-total-row"><span>Subtotal</span><strong>{formatBillingMoney(invoice.subtotal)}</strong></div><div className="billing-total-row"><span>Discount</span><strong>âˆ’ {formatBillingMoney(invoice.discount_amount)}</strong></div><div className="billing-total-row"><span>Tax</span><strong>{formatBillingMoney(invoice.tax_amount)}</strong></div><div className="billing-total-row grand"><span>Total</span><strong>{formatBillingMoney(invoice.total_amount)}</strong></div><div className="billing-total-row"><span>Paid</span><strong className="billing-balance-clear">{formatBillingMoney(invoice.paid_amount)}</strong></div><div className="billing-total-row due"><span>Balance</span><strong>{formatBillingMoney(invoice.balance_amount)}</strong></div>
...
function ReceiptPaper({ receipt }: { receipt: BillingReceipt }) {
  ...
  return <article className="billing-receipt-paper">
    ...
    <footer>Generated {formatBillingDateTime(receipt.generated_at)} Â· Electronically generated receipt</footer>
  </article>;
}

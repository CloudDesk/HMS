...
import { laboratoryApi, type LaboratoryResultPayload } from '../api/laboratory';
...
export function LaboratoryResultEntryPage() {
  ...
  const orderQuery = useQuery({ queryKey: ['laboratory', 'order', id], queryFn: () => laboratoryApi.get(id), enabled: Boolean(id) });
  const hasStoredResult = ['RESULT_ENTERED', 'VERIFIED', 'COMPLETED'].includes(orderQuery.data?.status ?? '');
  const resultQuery = useQuery({ queryKey: ['laboratory', 'result', id], queryFn: () => laboratoryApi.getResult(id), enabled: Boolean(id && hasStoredResult) });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormData) => {
      const payload: LaboratoryResultPayload = {
        result_items: values.result_items.map((item) => ({ ...item, unit: item.unit || null, reference_range: item.reference_range || null, comments: item.comments || null })),
        remarks: values.remarks || null,
      };
      return orderQuery.data?.status === 'IN_PROGRESS' ? laboratoryApi.enterResult(id, payload) : laboratoryApi.updateResult(id, payload);
    },
    onSuccess: async () => {
      toast.success('Laboratory results saved.');
      await queryClient.invalidateQueries({ queryKey: ['laboratory'] });
      navigate(`/laboratory/workspace?id=${id}`);
    },
    onError: (error) => toast.error(error instanceof ApiError ? error.message : 'Unable to save laboratory results.'),
  });
  ...
  return <div className="diagnostic-page laboratory"><section className="diagnostic-order-header card"><div><span className="eyebrow">Laboratory results</span><h2>{orderQuery.data.patient_name}</h2><p>{orderQuery.data.patient_number} · {orderQuery.data.doctor_name}</p></div><span className={`diagnostic-status status-${orderQuery.data.status.toLowerCase().replaceAll('_', '-')}`}>{orderQuery.data.status.replaceAll('_', ' ')}</span></section>
    <form className="card diagnostic-panel" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
      ...
      <div className="diagnostic-form-actions"><button className="btn-secondary" type="button" onClick={() => navigate(`/laboratory/workspace?id=${id}`)}>Back</button>{canEdit && !readOnly ? <button className="btn-primary" disabled={mutation.isPending || !canEnterResult} type="submit">{mutation.isPending ? 'Saving...' : 'Save Results'}</button> : null}</div>
    </form>
  </div>;
}

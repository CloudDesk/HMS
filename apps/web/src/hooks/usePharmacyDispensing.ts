...
export const usePharmacyDispensing = ({ branchId, search, status, page, limit, enabled = true }: PharmacyDispensingQuery) => {
  ...
  const listQuery = useQuery({ queryKey: ['pharmacy', 'dispensings', branchId, search, status, page, limit], queryFn: () => pharmacyDispensingService.list({ branch_id: branchId, search: search || undefined, status, page, limit }), enabled: enabled && Boolean(branchId) });

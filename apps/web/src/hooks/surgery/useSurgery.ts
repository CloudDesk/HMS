...
export const surgeryKeys = { all: ['surgery'] as const, recommendations: (params?: SurgeryListParams) => ['surgery', 'recommendations', params] as const, bookings: (params?: SurgeryListParams) => ['surgery', 'bookings', params] as const, alternatives: (params: Record<string, string>) => ['surgery', 'alternatives', params] as const };
export function useSurgery(params: SurgeryListParams, enabled: { recommendations: boolean; bookings: boolean }) {
  const client = useQueryClient(); const refresh = () => client.invalidateQueries({ queryKey: surgeryKeys.all });
  const recommendations = useQuery({ queryKey: surgeryKeys.recommendations(params), queryFn: () => surgeryService.recommendations(params), enabled: enabled.recommendations });
  const bookings = useQuery({ queryKey: surgeryKeys.bookings(params), queryFn: () => surgeryService.bookings(params), enabled: enabled.bookings });

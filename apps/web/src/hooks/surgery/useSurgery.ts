...
export function useSurgery(params: SurgeryListParams, enabled = true) {
  const client = useQueryClient(); const refresh = () => client.invalidateQueries({ queryKey: keys.all });
  const recommendations = useQuery({ queryKey: keys.recommendations(params), queryFn: () => surgeryService.recommendations(params), enabled });
  const bookings = useQuery({ queryKey: keys.bookings(params), queryFn: () => surgeryService.bookings(params), enabled });

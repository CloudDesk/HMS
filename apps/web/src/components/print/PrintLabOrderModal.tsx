...
export function PrintLabOrderModal({ order, patient, onClose }: Props) {
  ...
  return (
    <Modal onClose={onClose} open={Boolean(order)} title="Print Lab Order">
      <div style={{ position: 'relative' }}>
        <PrintReceiptLayout gridItems={gridItems} title="Official Laboratory Order">
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e2e8f0', textAlign: 'left', color: '#64748b' }}>
                  <th style={{ padding: '0.5rem' }}>Test Name</th>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1.5rem', padding: '0 2rem 2rem' }}>
          ...
          <button className="doc-btn primary" onClick={handlePrint} type="button"><i className="ph ph-printer" /> Print Lab Order</button>
        </div>
      </div>
    </Modal>
  );

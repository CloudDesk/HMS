...
export class AdmissionsConfigurationService {
  ...
  async listBeds(query: BedListQuery, actor: string, metadata: StatusActionMetadata = {}) { void metadata; await this.authorize(actor, query.branch_id); return this.repository.listBeds(query); }
  async getBed(id: string, branchId: string, actor: string, metadata: StatusActionMetadata = {}) { void metadata; await this.authorize(actor, branchId); const item = await this.repository.getBed(id, branchId); if (!item) throw new AppError('Bed not found', 404, 'BED_NOT_FOUND'); return item; }
  ...
  async summary(branchId: string, actor: string, metadata: StatusActionMetadata = {}) {
    await this.authorize(actor, branchId);
    await this.expireHolds(branchId, actor, metadata);
  ...
  async savePolicy(data: SaveAdmissionPolicyDTO, actor: string, metadata: StatusActionMetadata) {
    await this.authorize(actor, data.branch_id);
    let result;
    await executeTransaction(this.repository, async (session) => {
      result = await this.repository.savePolicy(data, actor, session);
      await this.repository.audit('admissions.policy.updated', actor, metadata, { branchId: data.branch_id, bedHoldDurationMinutes: data.bed_hold_duration_minutes }, session);
    });
    if (!result) throw new AppError('Admission policy could not be saved', 500, 'ADMISSION_POLICY_SAVE_FAILED');
    return result;
  }
  ...
  async closeHold(id: string, data: CloseBedHoldDTO, action: 'RELEASED' | 'CANCELLED', actor: string, metadata: StatusActionMetadata) {
    await this.authorize(actor, data.branch_id);
    let result;
    await executeTransaction(this.repository, async (session) => {
      result = await this.repository.closeHold(id, data.branch_id, action, data.reason.trim(), actor, session);
      if (!result) throw new AppError('Active bed hold not found or bed ownership changed', 409, 'BED_HOLD_CONFLICT');
      await this.repository.audit(action === 'RELEASED' ? 'admissions.bed_hold.released' : 'admissions.bed_hold.cancelled', actor, metadata, { holdId: id, bedId: result.bed_id, branchId: data.branch_id, reason: data.reason.trim() }, session);
    });
    return result;
  }
  ...
  async cancelTransfer(id: string, data: CancelBedTransferDTO, actor: string, metadata: StatusActionMetadata) {
    await this.authorize(actor, data.branch_id);
    let result;
    await executeTransaction(this.repository, async (session) => {
      result = await this.repository.cancelTransfer(id, data.branch_id, data.reason.trim(), actor, session);
      if (!result) throw new AppError('Pending bed transfer not found', 404, 'TRANSFER_NOT_FOUND');
      await this.repository.audit('admissions.bed_transfer.cancelled', actor, metadata, { transferId: id, admissionId: result.admission_id, reason: data.reason.trim() }, session);
    });
    return result;
  }
}

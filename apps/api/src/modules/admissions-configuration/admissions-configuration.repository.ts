...
const toBed = (item: BedRecord & { ward?: { name: string } | null }): Bed => ({ id: item._id.toString(), branch_id: item.branchId.toString(), ward_id: item.wardId.toString(), ward_name: item.ward?.name ?? '', bed_number: item.bedNumber, bed_category: item.bedCategory, room_number: item.roomNumber ?? null, status: item.status, created_at: item.createdAt, updated_at: item.updatedAt });
...
export class AdmissionsConfigurationRepository {
  ...
  async getWard(id: string, branchId: string) { return WardModel.findOne({ _id: oid(id), branchId: oid(branchId) }).lean<WardRecord>(); }
  ...
  async updateWard(id: string, data: UpdateWardDTO, userId: string, session?: ClientSession) { const record = await WardModel.findOneAndUpdate({ _id: oid(id), branchId: oid(data.branch_id) }, { $set: { name: data.name?.trim(), wardType: data.ward_type?.trim(), floor: data.floor?.trim(), description: data.description?.trim() || null, updatedBy: oid(userId) } }, { returnDocument: 'after', lean: true, runValidators: true, session }).lean<WardRecord>(); return record ? toWard(record) : null; }
  ...
  async updateBed(id: string, branchId: string, data: UpdateBedDTO, userId: string, session?: ClientSession) { const record = await BedModel.findOneAndUpdate({ _id: oid(id), branchId: oid(branchId) }, { $set: { bedNumber: data.bed_number?.trim().toUpperCase(), bedCategory: data.bed_category?.trim(), roomNumber: data.room_number?.trim() || null, updatedBy: oid(userId) } }, { returnDocument: 'after', lean: true, runValidators: true, session }).lean<BedRecord>(); return record ? this.getBed(record._id.toString(), branchId) : null; }

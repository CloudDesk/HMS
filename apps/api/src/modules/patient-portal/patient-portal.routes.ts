...
import { setRefreshTokenCookie } from '../../shared/security/auth-cookies.js';
...
export const registerPatientPortalRoutes = async (app: FastifyInstance, services: ServiceRegistry) => {
  ...
  app.post<{ Body: VerifyOtpBody }>('/api/patient-portal/otp/verify', async (request) => {
    ...
    const result = await services.patientPortal.verifyOtp(parsed.data.phone, parsed.data.otp);
  ...
  app.post<{ Body: RegisterBody }>('/api/patient-portal/signup', async (request, reply) => {
    ...
    if (parsed.data.registration_token) {
      await services.patientPortal.verifyAndConsumeRegistrationToken(parsed.data.phone, parsed.data.registration_token);
    ...
    return reply.status(201).send(ok({ account, ...session }));
  });

  app.post<{ Body: OtpLoginBody }>('/api/patient-portal/login/otp', async (request, reply) => {
    ...
    const result = await services.auth.loginPatientWithOtp(parsed.data, metadataFromRequest(request));

...
export const registerInpatientAdmissionRoutes = async (app: FastifyInstance, services: ServiceRegistry) => {
  ...
  app.get('/api/admissions/inpatients/:id/prescription', { preHandler: requirePermission(services, 'OPD', 'OPD Prescription', 'View') }, async (request) => { const params = parse(inpatientAdmissionIdSchema, request.params); const query = parse(clinicalContextBranchSchema, request.query); return ok(await services.inpatientAdmissions.getPrescription(params.id, query.branch_id, request.user!.id)); });
  ...
  app.get('/api/admissions/inpatients/:id/round-notes', { preHandler: requirePermission(services, 'Admissions', 'Inpatient Admissions', 'View') }, async (request) => { const params = parse(inpatientAdmissionIdSchema, request.params); const query = parse(clinicalContextBranchSchema, request.query); return ok(await services.inpatientAdmissions.listRoundNotes(params.id, query.branch_id, request.user!.id)); });
  app.post('/api/admissions/inpatients/:id/round-notes', { preHandler: requirePermission(services, 'Admissions', 'Inpatient Admissions', 'Create') }, async (request, reply) => { const params = parse(inpatientAdmissionIdSchema, request.params); const query = parse(clinicalContextBranchSchema, request.query); return reply.status(201).send(ok(await services.inpatientAdmissions.createRoundNote(params.id, query.branch_id, parse(createInpatientRoundNoteSchema, request.body), request.user!.id, metadata(request)))); });
  app.get('/api/admissions/inpatients/:id/vitals', { preHandler: requirePermission(services, 'Admissions', 'Inpatient Admissions', 'View') }, async (request) => { const params = parse(inpatientAdmissionIdSchema, request.params); const query = parse(clinicalContextBranchSchema, request.query); return ok(await services.inpatientAdmissions.listVitals(params.id, query.branch_id, request.user!.id)); });
  app.post('/api/admissions/inpatients/:id/vitals', { preHandler: requirePermission(services, 'Admissions', 'Inpatient Admissions', 'Create') }, async (request, reply) => { const params = parse(inpatientAdmissionIdSchema, request.params); const query = parse(clinicalContextBranchSchema, request.query); return reply.status(201).send(ok(await services.inpatientAdmissions.createVital(params.id, query.branch_id, parse(createInpatientVitalSchema, request.body), request.user!.id, metadata(request)))); });

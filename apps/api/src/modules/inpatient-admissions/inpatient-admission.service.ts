...
export class InpatientAdmissionService {
  ...
  async list(query: InpatientAdmissionListQuery, actor: string) { await this.authorize(actor, query.branch_id); return this.repository.list(query); }
  async get(id: string, branchId: string, actor: string) { await this.authorize(actor, branchId); const item = await this.repository.getById(id, branchId); if (!item) throw new AppError('Inpatient admission not found', 404, 'ADMISSION_NOT_FOUND'); return item; }
  async listRequests(query: AdmissionRequestListQuery, actor: string) { await this.authorize(actor, query.branch_id); return this.repository.listRequests(query); }
  async getRequest(id: string, branchId: string, actor: string) { await this.authorize(actor, branchId); const item = await this.repository.getRequest(id, branchId); if (!item) throw new AppError('Admission request not found', 404, 'ADMISSION_REQUEST_NOT_FOUND'); return item; }
  ...
  async confirmRequest(id: string, branchId: string, data: ConfirmAdmissionRequestDTO, actor: string, metadata: AdmissionRequestMetadata) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        if (request.status === 'CONFIRMED' && request.admission_id) { result = request; return; }

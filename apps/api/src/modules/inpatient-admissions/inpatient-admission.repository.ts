...
export class InpatientAdmissionRepository {
  async hasActiveAdmission(patientId: string, session?: ClientSession) {
    const q = InpatientAdmissionModel.exists({ patientId: oid(patientId), status: 'ADMITTED' });
  ...
  async references(data: CreateInpatientAdmissionDTO, session?: ClientSession) {
    const qWard = WardModel.findOne({ _id: oid(data.ward_id), branchId: oid(data.branch_id), status: 'ACTIVE' });
    if (session) qWard.session(session);
    const [patient, doctor, department, ward] = await Promise.all([
      PatientModel.findOne({ _id: oid(data.patient_id), status: 'ACTIVE', deletedAt: null }).lean(),
      DoctorModel.findOne({ _id: oid(data.admitting_doctor_id), branchId: oid(data.branch_id), status: 'ACTIVE', deletedAt: null }).lean(),
      DepartmentModel.findOne({ _id: oid(data.department_id), status: 'ACTIVE', deletedAt: null }).lean(),
      qWard.lean(),
    ]);
    return { patient, doctor, department, ward };
  }
  async create(data: CreateInpatientAdmissionDTO, refs: { patientNumber: string; patientName: string; doctorName: string; departmentName: string }, userId: string, session?: ClientSession, source: { requestId?: string | null; sourceType?: AdmissionSourceType; sourceId?: string | null } = {}) {
    const opts = session ? { session } : undefined;
    const created = await InpatientAdmissionModel.create([{ admissionNumber: `ADM-${Date.now()}-${Math.floor(Math.random() * 1000)}`, patientId: oid(data.patient_id), patientNumber: refs.patientNumber, patientName: refs.patientName, branchId: oid(data.branch_id), wardId: oid(data.ward_id), bedId: oid(data.bed_id), admittingDoctorId: oid(data.admitting_doctor_id), admittingDoctorName: refs.doctorName, departmentId: oid(data.department_id), departmentName: refs.departmentName, admissionDate: new Date(data.admission_date), admissionType: data.admission_type, reason: data.reason, notes: data.notes ?? null, status: 'ADMITTED', requestId: source.requestId ? oid(source.requestId) : null, sourceType: source.sourceType ?? 'DIRECT', sourceId: source.sourceId ? oid(source.sourceId) : null, createdBy: oid(userId), updatedBy: oid(userId) }], opts);
    const first = created[0];
    if (!first) throw new Error('Admission create returned no record');
    return toDto(first.toObject() as InpatientAdmissionFields & { _id: Types.ObjectId });
  }

  async requestReferences(data: CreateAdmissionRequestDTO, session?: ClientSession) {
    const qPatient = PatientModel.findOne({ _id: oid(data.patient_id), status: 'ACTIVE', deletedAt: null });
    const qDoctor = DoctorModel.findOne({ _id: oid(data.recommending_doctor_id), status: 'ACTIVE', deletedAt: null });
    const qDepartment = DepartmentModel.findOne({ _id: oid(data.department_id), status: 'ACTIVE', deletedAt: null });
    if (session) {
      qPatient.session(session);
      qDoctor.session(session);
      qDepartment.session(session);
    }
    const [patient, doctor, department] = await Promise.all([qPatient.lean(), qDoctor.lean(), qDepartment.lean()]);
  ...
  async createRequest(data: CreateAdmissionRequestDTO, refs: { patientNumber: string; patientName: string; doctorName: string; departmentName: string; sourceReference?: string | null }, userId: string, session?: ClientSession) {
    const opts = session ? { session } : undefined;
    const created = await AdmissionRequestModel.create([{ requestNumber: `AR-${Date.now()}-${Math.floor(Math.random() * 1000)}`, patientId: oid(data.patient_id), patientNumber: refs.patientNumber, patientName: refs.patientName, branchId: oid(data.branch_id), departmentId: oid(data.department_id), departmentName: refs.departmentName, recommendingDoctorId: oid(data.recommending_doctor_id), recommendingDoctorName: refs.doctorName, sourceType: data.source_type, sourceId: data.source_id ? oid(data.source_id) : null, sourceReference: refs.sourceReference ?? null, activeSourceKey: data.source_id ? `${data.source_type}:${data.source_id}` : null, admissionType: data.admission_type, priority: data.priority, reason: data.reason, notes: data.notes ?? null, status: 'PENDING_VALIDATION', createdBy: oid(userId), updatedBy: oid(userId) }], opts);
    const first = created[0];
    if (!first) throw new Error('Admission request create returned no record');
    return toRequest(first.toObject() as AdmissionRequestFields & { _id: Types.ObjectId });
  }
  async listRequests(query: AdmissionRequestListQuery) { const page = query.page ?? 1; const limit = query.limit ?? 20; const filter: Record<string, unknown> = { branchId: oid(query.branch_id) }; if (query.status) filter.status = query.status; if (query.source_type) filter.sourceType = query.source_type; if (query.patient_id) filter.patientId = oid(query.patient_id); if (query.search) { const escaped = query.search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); const value = new RegExp(escaped, 'i'); filter.$or = [{ requestNumber: value }, { patientNumber: value }, { patientName: value }]; } const [items, total] = await Promise.all([AdmissionRequestModel.find(filter).sort({ createdAt: -1, _id: -1 }).skip((page - 1) * limit).limit(limit).lean<(AdmissionRequestFields & { _id: Types.ObjectId })[]>(), AdmissionRequestModel.countDocuments(filter)]); return { data: items.map(toRequest), meta: meta(total, page, limit) }; }

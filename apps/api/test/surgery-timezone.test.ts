...
describe('SurgeryService - Timezone Handling', () => {
  ...
  it('Test A - Local positive case (Non-UTC timezone, booking inside local working hours)', async () => {
    ...
    await expect(
      (service as unknown as { validateSchedule: (...args: unknown[]) => Promise<void> }).validateSchedule('doc1', start, end, 'srv1', 1)
    ).resolves.toBeUndefined();
  });

  it('Test B - Local negative case (Non-UTC timezone, booking outside local working hours)', async () => {
    const service = createMockService('Asia/Kolkata');
  ...
  it('Test C - UTC/local weekday boundary (UTC date and local date differ)', async () => {
    ...
    await expect(
      (serviceAuckland as unknown as { validateSchedule: (...args: unknown[]) => Promise<void> }).validateSchedule('doc1', start, end, 'srv1', 1)
    ).resolves.toBeUndefined();
  });

  it('Test D - DST (DST-aware timezone correctly adjusts local clock time)', async () => {
    ...
    await expect(
      (service as unknown as { validateSchedule: (...args: unknown[]) => Promise<void> }).validateSchedule('doc1', startEDT, endEDT, 'srv1', 1)
    ).resolves.toBeUndefined();

    await expect(
      (service as unknown as { validateSchedule: (...args: unknown[]) => Promise<void> }).validateSchedule('doc1', startEST, endEST, 'srv1', 1)
    ).resolves.toBeUndefined();
  });

  it('Test E - UTC (UTC timezone preserves existing behavior)', async () => {

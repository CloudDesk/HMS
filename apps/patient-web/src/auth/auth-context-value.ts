...
export type AuthContextValue = { status: AuthStatus; user: AuthUser | null; authError: string | null; login: (identifier: string, password: string) => Promise<void>; loginWithOtp: (phone: string, otp: string) => Promise<void>; activateGuardian: (input: GuardianActivationInput) => Promise<void>; restoreSession: () => Promise<void>; logout: () => Promise<void>; clearAuthError: () => void };

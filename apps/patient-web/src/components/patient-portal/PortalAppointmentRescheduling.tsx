...
export function PortalAppointmentRescheduling({
  appointment,
  onSaved,
  onCancel,
}: {
  ...
  const eligibility = useQuery({
    queryKey: portalQueryKeys.rescheduleEligibility(appointment.id),
  ...
  if (eligibility.isLoading)
    return (
      <div className="portal-empty">
        ...
        <strong>Checking reschedule eligibility…</strong>
      </div>
    );
  if (eligibility.isError || !eligibility.data)
    return (
      <div className="portal-empty">
        ...
        <strong>Eligibility could not be checked</strong>
        <button onClick={() => void eligibility.refetch()} type="button">
          Try again
        </button>
      </div>
    );
  if (!eligibility.data.eligible)
    return (
      <div className="portal-empty">
        ...
        <strong>Rescheduling is not available</strong>
        <span>{eligibility.data.reason}</span>
    ...
    >
        ...
        <div>
          ...
          <div className="apt-meta">
            <span className="apt-chip">
              ...
              {localDateValue(new Date(appointment.appointment_date))}
            </span>
            <span className="apt-chip">
      ...
      <section>
        <div className="portal-form-section-title">
          <span>1</span>
          <div>
            <strong>Choose the new care location</strong>
            <small>You may keep the same doctor or choose another available doctor.</small>
          </div>
        </div>
          ...
          <label className="wide">
            ...
            <select
              ...
              {...form.register('doctor_id', {
                onChange: () => form.setValue('appointment_date', today),
              })}
            >
              <option value="">
                {!departmentId
              ...
              {doctors.data?.data.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.display_name} · {item.specialization}
                </option>
              ))}
            </select>
            {doctors.isError ? (
      ...
      <section>
        <div className="portal-form-section-title">
          ...
          <div>
            ...
            <small>The new slot is checked again when you confirm.</small>
          </div>
        </div>
        ...
        <div className="portal-slot-area">
          {!doctorId || !appointmentDate ? (
            <p>
              <i className="ph ph-info" /> Select a doctor and date to see available times.
            </p>
          ) : slots.isLoading ? (
            <p>
            ...
            <p>
              <i className="ph ph-calendar-x" /> No open times remain on this date.
            </p>
          )}
        </div>
      </section>
      ...
      <footer className="portal-form-actions">
        ...
        >
          ...
          {mutation.isPending ? 'Rescheduling…' : 'Confirm new appointment'}
        </button>
      </footer>
    </form>
  );

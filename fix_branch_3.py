...
def fix():
    ...
    code = code.replace("import { useEffect, useState, type FormEvent } from 'react';", "import { useEffect, useState } from 'react';\nimport { useForm } from 'react-hook-form';\nimport { zodResolver } from '@hookform/resolvers/zod';\nimport { z } from 'zod';")
...
"""
    ...
    def r1(m):
        return f"{m.group(2) or ''}\n                    {m.group(3) or ''}\n                    {{...branchForm.register('{m.group(1)}')}}"
    code = re.sub(r'onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value \}\)\}\s*(required)?\s*(type="text"|type="email")?\s*value=\{form\.[a-zA-Z]+\}', r1, code)

    def r2(m):
        return f"{m.group(2) or ''}\n                    {{...branchForm.register('{m.group(1)}')}}"
    code = re.sub(r'onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value as ApiBranchStatus \}\)\}\s*(required)?\s*value=\{form\.[a-zA-Z]+\}', r2, code)

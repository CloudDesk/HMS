...
"""
    code = code.replace("type BranchFormState = {", schema + "\n/*")
    code = code.replace("  status: ApiBranchStatus;\n};\n\nconst emptyForm: BranchFormState = {", "*/\n\n/*const emptyForm: BranchFormState = {")
    ...
    # JSX Bindings
    code = code.replace('<form className="modal-form" id="branch-management-form" onSubmit={handleSave}>', '<form className="modal-form" id="branch-management-form" onSubmit={branchForm.handleSubmit(handleSave)}>')
    
    # Remove manual state assignments
    code = re.sub(r'\s*onChange=\{\(e\) => setForm\(\{ \.\.\.form, [a-zA-Z]+: e\.target\.value \}\)\}\n\s*(?:required\n)?\s*type="text"\n\s*value=\{form\.[a-zA-Z]+\}', lambda m: f"\n                    type=\"text\"\n                    {{...branchForm.register('{m.group(0).split('form.')[1].split('}')[0]}')}}", code)
    code = re.sub(r'\s*onChange=\{\(e\) => setForm\(\{ \.\.\.form, [a-zA-Z]+: e\.target\.value \}\)\}\n\s*type="email"\n\s*value=\{form\.[a-zA-Z]+\}', lambda m: f"\n                    type=\"email\"\n                    {{...branchForm.register('{m.group(0).split('form.')[1].split('}')[0]}')}}", code)
    code = re.sub(r'\s*onChange=\{\(e\) => setForm\(\{ \.\.\.form, [a-zA-Z]+: e\.target\.value as ApiBranchStatus \}\)\}\n\s*required\n\s*value=\{form\.[a-zA-Z]+\}', lambda m: f"\n                    {{...branchForm.register('{m.group(0).split('form.')[1].split('}')[0]}')}}", code)

    # Some form group fixes if needed. In branch it's form.code, form.name etc. Let's make regex robust.
    code = re.sub(r'onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value \}\)\}\s*(required)?\s*type="(text|email)"\s*value=\{form\.[a-zA-Z]+\}', r'\2\n                    type="\3"\n                    {...branchForm.register("\1")}', code)
    code = re.sub(r'onChange=\{\(e\) => setForm\(\{ \.\.\.form, ([a-zA-Z]+): e\.target\.value as ApiBranchStatus \}\)\}\s*(required)?\s*value=\{form\.[a-zA-Z]+\}', r'\2\n                    {...branchForm.register("\1")}', code)
    
    # Add error rendering manually if we want or just remove old validation
    # the original UI doesn't seem to have individual field errors for Branch.

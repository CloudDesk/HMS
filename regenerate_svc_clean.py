...
subprocess.run(["git", "checkout", "apps/web/src/pages/ServiceCataloguePage.tsx"], check=True)

with open('apps/web/src/pages/ServiceCataloguePage.tsx', 'r', encoding='utf-8') as f:
    text = f.read()

# Make sure we don't accidentally nuke the whole file!
import_str = "import { useServiceCatalogueFeature, type SortColumn, type SortDirection } from '../hooks/services/useServiceCatalogueFeature';\n"
text = text.replace("import { useAuth } from '../auth/useAuth';", import_str)
text = text.replace("import { hasPermission } from '../auth/access-control';\n", "")
text = text.replace("type SortColumn = 'code' | 'name' | 'standard_price' | 'created_at';\ntype SortDirection = 'asc' | 'desc';\n", "")
text = text.replace("type LookupPage<T> = {\n  data: T[];\n  meta: { totalPages: number };\n};\n\n", "")
...
text = text.replace("disabled={forbidden || !hasExportPermission || submitting}", "disabled={forbidden || !canExport || submitting}")
text = text.replace("disabled={forbidden || !hasCreatePermission || submitting}", "disabled={forbidden || !canCreate || submitting}")
...
text = text.replace("import { useCreateService, useUpdateService, useUpdateServiceStatus, useDeleteService } from '../hooks/services/useServices';\n", "")
text = text.replace("import { branchesApi, type BranchResponse } from '../api/branches';\n", "import { type BranchResponse } from '../api/branches';\n")
text = text.replace("import { departmentsApi, type DepartmentResponse } from '../api/departments';\n", "import { type DepartmentResponse } from '../api/departments';\n")
